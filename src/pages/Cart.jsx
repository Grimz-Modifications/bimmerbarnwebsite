      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-8 py-10">
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => <div key={i} className="h-24 bg-neutral-900 animate-pulse" />)}
          </div>
        ) : cartItems.length === 0 ? (
              ...
              <AnimatePresence>
                {cartItems.map((item) => (
                  <motion.div
                    ...
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0, x: -40 }}
                    className="flex items-center gap-4 bg-neutral-950 border border-neutral-800 p-4"
                  >
                    ...
                    <div className="flex items-center border border-neutral-700">
                      <button onClick={() => updateMutation.mutate({ id: item.id, data: { quantity: Math.max(1, (item.quantity || 1) - 1) } })} className="px-3 py-2 text-neutral-500 hover:text-white hover:bg-neutral-900 transition-colors">
                        <Minus className="w-3 h-3" />
                      </button>
                      <span className="px-3 py-2 text-white text-sm font-bold min-w-[36px] text-center">{item.quantity || 1}</span>
                      <button onClick={() => updateMutation.mutate({ id: item.id, data: { quantity: (item.quantity || 1) + 1 } })} className="px-3 py-2 text-neutral-500 hover:text-white hover:bg-neutral-900 transition-colors">
                        <Plus className="w-3 h-3" />
                      </button>
                    </div>
                    <p className="text-white font-bold text-sm w-20 text-right">${(item.price * (item.quantity || 1)).toFixed(2)}</p>
                    <button onClick={() => deleteMutation.mutate(item.id)} className="text-neutral-600 hover:text-white transition-colors p-1">
